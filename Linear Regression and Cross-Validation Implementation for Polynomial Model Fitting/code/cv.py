...
# Description: The functions helps in implementing the generalized matrix form of the normal 
# equation for linear regression, implementation and use of cross-validation including the simple regression results. 
# Collabrators: Rachel & Nees

# ISTA 421 / INFO 521 Fall 2023 HW 2, Exercise 3
...
#### References/Citation####
# Textbook - An introduction to Statistical Learning -- https://hastie.su.domains/ISLR2/ISLRv2_corrected_June_2023.pdf
# Textbook - A first course in machine learning (Hardcopy)
...
RUN_DEMO = True
RUN_EXERCISE_3_5FOLD = True
RUN_EXERCISE_3_LOOCV = True

# -----------------------------------------------------------------------------
# Global paths -- Do NOT edit these paths!
# -----------------------------------------------------------------------------

DATA_ROOT = '../data'
DATA_SYNTH2023 = os.path.join(DATA_ROOT, 'synthdata2023.csv')

FIGURES_ROOT = '../figures'
PATH_TO_SYNTH_5FOLDCV_FIG = os.path.join(FIGURES_ROOT, 'synthetic2023-5fold-CV.pdf')
PATH_TO_SYNTH_LOOCV_FIG = os.path.join(FIGURES_ROOT, 'synthetic2023-LOOCV.pdf')
PATH_TO_SYNTH_5FOLDCV_FIG_2 = os.path.join(FIGURES_ROOT, 'synthetic2021-5fold-CV-2.pdf')
PATH_TO_SYNTH_LOOCV_FIG_2 = os.path.join(FIGURES_ROOT, 'synthetic2021-LOOCV_2.pdf')
...
def permute_rows(X, P=None):
    ...
    using random_permutation_matrix().
    :param X: 2-d array
...
# NOTE: The following is ONLY used by the demo example code.
#       For exercise 3 of hw2, you will be loading the specified data from file.
...
def run_cv(K, maxorder, x, t, independent_testx, independent_testt,
    ...
    for p in range(maxorder + 1):
        ...
        for fold in range(K):
            ...
            a = numpy.linalg.inv(numpy.dot(trainX.T, trainX))
            #print (a)
            b = numpy.dot(traint.T,trainX)
...
def fitpoly(x, t, model_order, verbose_p=False):
    ...
    #### YOUR CODE HERE ####
    a = numpy.linalg.inv(numpy.dot(X.T, X))
    b = numpy.dot(t.T,X)

    w = numpy.dot(a,b)  # Calculate w vector (as an numpy.array)
...
def ex3_5(K, maxorder, x, t, randomize_data=False, title='CV', save_path=None):
    """
    Implements the core of the cross-validation demonstration
    from FCML Ch 1, pp.31-32.
    :param K: Number of folds
    ...
    plot_cv_results(log_mean_train_loss, log_mean_cv_loss,
                    log_scale_p=True, plot_title=title,filepath=save_path)

    best_model = fitpoly(x,t,best_poly)
    # Uncomment to plot direct-scale mean loss results
    #plot_cv_results(mean_train_loss, mean_cv_loss, log_scale_p=False)
    
    return best_poly, min_mean_log_cv_loss , best_model
...
def exercise_3_5fold(data_path, figure_path, seed):
    # Load the data
    ...
    K = 5
    #call best poly, mean log and W
    best_poly, min_mean_log_cv_loss, w = \
        ex3_5( K, 7, x, t, True, f'5 Fold CV' , figure_path) #To get the plot
    plot_data_and_model(x, t, w)
    ...
    #       follows:
    #   k5_best_poly, k5_best_CVtest_log_MSE_loss, k5_best_model_w = run_cv_solution(...)
    #       rather than individually, as above.

    print('\n===== 5-fold CV')
    print(f'best model order: {k5_best_model_order}')
    print(f'best model log MSE CV Test loss: {k5_best_CVtest_log_MSE_loss}')
    print(f'best model parameters: {k5_best_model_w}')

    return k5_best_model_order, k5_best_CVtest_log_MSE_loss, k5_best_model_w
...
def exercise_3_LOOCV(data_path, figure_path_LOOCV, seed):
    # Load the data
    data = read_data(data_path, ',')
    x = data[:, 0]  # extract x (slice first column)
    t = data[:, 1]  # extract t (slice second column)
    ...
    #call best poly, mean log and W
    best_poly, min_mean_log_cv_loss, w = \
        ex3_5( K, 7, x, t, True, f'LOOCV' , figure_path_LOOCV) #for plot
    plot_data_and_model(x, t, w)
    
    #plt.savefig(PATH_TO_SYNTH_LOOCV_FIG_2, format='pdf')#just to save the plot

    ### YOUR CODE HERE
    # Perform Leave-One-Out cross-validation (LOOCV):
    # Calculate the best polynomial model order as loocv_best_model_order
    loocv_best_model_order = best_poly
    # Calculate the log MSE CV test loss for the best polynomial model
    #       order on the synthetic data
    loocv_best_CVtest_log_MSE_loss = min_mean_log_cv_loss
    # Report the best-fit parameters w for teh best model order
    loocv_best_model_w = w
    ...
    #       at once, such as:
    #   loocv_best_model_order, loocv_best_CVtest_log_MSE_loss, loocv_best_model_w = run_cv_solution(...)
...
if __name__ == '__main__':
    ...
    if RUN_EXERCISE_3_5FOLD:
        exercise_3_5fold(DATA_SYNTH2023, PATH_TO_SYNTH_5FOLDCV_FIG, seed=29)
        plt.show()
    if RUN_EXERCISE_3_LOOCV:
        exercise_3_LOOCV(DATA_SYNTH2023, PATH_TO_SYNTH_LOOCV_FIG, seed=29)
        plt.show()
